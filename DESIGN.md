...
— near-black, grain-textured, generatively lit from below — never changes
...
- Flat by default; glass/blur panels reserved for the two "you are inside the room" moments
...
- **Body — Editorial** (weight 400, `clamp(1.15rem, 1rem + 0.7vw, 1.4rem)`, line-height 1.75, serif, capped at `34rem`/~55ch measure): Origins, Beliefs, Practice, Symbols — third-person factual sections.
- **Body — Immersive** (weight 300, `clamp(1.3rem, 1.1rem + 1.1vw, 1.9rem)`, line-height 1.55, tracking -0.01em, sans, capped at `46rem` measure): "A Day / A Life" — second-person narrative, deliberately lighter-weight and larger than editorial body text so it reads as spoken rather than written.
...
- **Immersive panel lift** (`box-shadow: 0 40px 80px -40px rgba(0,0,0,0.8)`): the one true elevation shadow in the system, reserved for the `ImmersiveModule` glass panel.

### Named Rules
**The Two-Glass Rule.** `backdrop-filter: blur()` is reserved for exactly two
UI moments — the Hero's disclaimer box and the `ImmersiveModule` narrative
panel — plus the floating `SoundToggle` control. Everywhere else the system is
flat. Glass is a signal ("you've entered the immersive layer" or "this is a
floating control"), not a texture applied for visual interest.
...
- **Editorial:** flat, `34rem` measure, serif body, `strong` set in the accent color, `em` nudged toward the accent via `color-mix`. No background treatment — it's page background straight through.
- **Immersive:** the one glass-panel + shadow treatment (see Elevation), sans body, `46rem` measure, its own `Backdrop` re-rendered behind the panel using the section's pattern (forced to `geometric` when `data.aniconic` is true).
- **Quote:** flat, `44rem` measure, serif italic, a 3px accent-tinted left border on the `blockquote` as the only structural marker, `cite` set in small-caps-style sans/muted.
...
- **Don't** use `backdrop-filter` blur decoratively outside the two sanctioned glass moments (Hero disclaimer, Immersive panel) plus the floating Sound Toggle.
